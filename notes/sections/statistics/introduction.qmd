...
Please watch the videos by 3Blue1Brown. His videos will be much better than my explanation. In his videos, he explains the architecture of GPT (Generative Pre-trained Transformer). I would not say "highly recommend". I would say "must watch" if you want to understand how LLMs work. The seminal paper is "Attention is All You Need" by Google Brain. The paper introduces the transformer architecture, which includes two key components: the attention mechanism and the encoder-decoder structure. GPT borrows the attention mechanism from the transformer architecture and simplifies the encoder-decoder structure to a decoder-only structure. Please watch the following videos by 3Blue1Brown to understand how LLMs work:

1. [Transformers, the tech behind LLMs | Deep Learning Chapter 5](https://www.youtube.com/watch?v=wjZofJX0v4M&list=PLZHQObOWTQDM4E-dwvbnQTiyKDO-y9T2t&index=4)
2. [Attention in transformers, step-by-step | Deep Learning Chapter 6](https://www.youtube.com/watch?v=eMlx5fFNoYc&list=PLZHQObOWTQDM4E-dwvbnQTiyKDO-y9T2t&index=3)
3. [How might LLMs store facts | Deep Learning Chapter 7](https://www.youtube.com/watch?v=9-Jl0dxWQs8&list=PLZHQObOWTQDM4E-dwvbnQTiyKDO-y9T2t&index=2)

For training LLMs, the seminal paper is [Training language models to follow instructions with human feedback](https://arxiv.org/abs/2203.02155) by OpenAI. They introduce the concept of reinforcement learning with human feedback (RLHF) to train LLMs. They introduced the canonical three-stage training process for LLMs: 

1. Pre-training: The model is trained on a large corpus of text data using unsupervised learning to learn the statistical properties of language.
2. Fine-tuning: The model is fine-tuned on a smaller, task-specific dataset. The dataset consists of input-output pairs, where the input is a prompt and the output is the desired response. 
3. Reinforcement learning with human feedback (RLHF): The model produces multiple responses to a given prompt, and human annotators rank the responses based on their quality. The model is then trained to maximize the reward from the human feedback using reinforcement learning.

The term "reinforcement learning" might be confusing. However, the key idea is very simple. The model is trained to maximize the reward function or minimize the loss function, without any sample data. The sample data is generated by the model itself. In RLHF, the LLM generates multiple responses to a given prompt, and human annotators rank the responses based on their quality. 

## Bayesian Statistics

In finance, we rarely use Bayesian statistics in empirical work. However, Bayesian statistics is widely used in modeling, such as in asset pricing models with sentiment. As bayesian updates is considered the rational belief updating process, so it is widely used in modeling to describe how investors update their beliefs about the market. If there's sentiment, investors may update their beliefs deviating from the bayesian updating process.

Bayesian statistics treats an unknown parameter as a random variable. Before observing data, uncertainty about a parameter $\theta$ is described by a **prior** distribution $p(\theta)$. After observing data $\boldsymbol{y}$, Bayes' rule combines the prior with the likelihood:
$$
p(\theta\mid\boldsymbol{y})
=\frac{p(\boldsymbol{y}\mid\theta)p(\theta)}{p(\boldsymbol{y})}
\propto p(\boldsymbol{y}\mid\theta)p(\theta).
$$
The resulting **posterior** distribution summarizes what we know about $\theta$ after seeing the data.

Let's use a simple example to illustrate the idea of Bayesian statistics. Suppose we want to estimate the mean return of a stock. We have some prior belief about the mean return
$$
\mu\sim\mathcal{N}(\mu_0,\sigma_0^2),
$$
meaning that we believe the mean return is normally distributed with mean $\mu_0$ and variance $\sigma_0^2$. If we are very confident about our prior belief, then $\sigma_0^2$ is small. If we are not very confident about our prior belief, then $\sigma_0^2$ is large.

Now we hear a analyst's forecasts about the stock. We can combine our prior belief with the analyst's forecasts to update our belief about the mean return. The analyst's forecasts can be modeled as a normal distribution with mean $\mu$ and variance $\sigma_a^2$:
$$
y\mid\mu\sim\mathcal{N}(\mu,\sigma_a^2).
$$
This means that suppose the true mean return is $\mu$, then the analyst's forecasts $y$ are normally distributed around $\mu$ with variance $\sigma_a^2$. If the analyst's forecasts is very accurate, then $\sigma_a^2$ is small. If the analyst's forecasts is not very accurate, then $\sigma_a^2$ is large.

We need to find a way to combine our prior belief and the analyst's forecasts. Intuitively, if we are very confident about our prior belief, then we should give more weight to our prior belief. If we are not very confident about our prior belief but the analyst's forecasts is very accurate, then we should give more weight to the analyst's forecasts. We shall see that Bayesian statistics provides a formal way to combine our prior belief and the analyst's forecasts, and the resulting posterior distribution reflects our intuition.

Based on Bayes' rule, the posterior distribution of $\mu$ given the analyst's forecasts $y$ is
$$
p(\mu\mid y)= \frac{p(y\mid\mu)p(\mu)}{p(y)} \propto p(y\mid\mu)p(\mu).
$$
The reason we can ignore the denominator $p(y)$ is that it does not depend on $\mu$. Let's calculate the right-hand side of the above equation. 
$$ 
\begin{aligned}
p(y\mid\mu)p(\mu) 
&= \frac{1}{\sqrt{2\pi\sigma_a^2}}\exp\left(-\frac{(y-\mu)^2}{2\sigma_a^2}\right) \cdot \frac{1}{\sqrt{2\pi\sigma_0^2}}\exp\left(-\frac{(\mu-\mu_0)^2}{2\sigma_0^2}\right)\\
&\propto \exp\left(-\frac{(\mu-y)^2}{2\sigma_a^2}-\frac{(\mu-\mu_0)^2}{2\sigma_0^2}\right)\\
&\propto \exp\left(-\frac{1}{2}\frac{(\mu-\phi)^2}{\sigma^2}\right)\\
\end{aligned}
$$
where 
$$
\phi = \frac{\sigma_0^2}{\sigma_0^2+\sigma_a^2}y + \frac{\sigma_a^2}{\sigma_0^2+\sigma_a^2}\mu_0, \qquad \sigma^2 = \frac{\sigma_0^2\sigma_a^2}{\sigma_0^2+\sigma_a^2}.
$$
Firstly, the posterior distribution is also a normal distribution. Secondly, the posterior mean $\phi$ is a weighted average of the prior mean $\mu_0$ and the analyst's forecasts $y$. If we are very confident about our prior belief (e.g. $\sigma_0^2=0$), then the posterior mean is equal to the prior mean $\mu_0$. If we are not very confident about our prior belief but the analyst's forecasts is very accurate (e.g. $\sigma_a^2=0$), then the posterior mean is equal to the analyst's forecasts $y$. Thirdly, the posterior variance $\sigma^2$ is smaller than both the prior variance $\sigma_0^2$ and the analyst's forecasts variance $\sigma_a^2$. This means that after observing the analyst's forecasts, we are more confident about our belief about the mean return.

Now suppose we have $N$ independent observations $y_1,\ldots,y_N$ with known variance $\sigma_a^2$:
$$
y_i\mid\mu\sim\mathcal{N}(\mu,\sigma_a^2),
$$
How do we combine our prior belief and the $N$ observations to update our belief about the mean return? The posterior distribution of $\mu$ given the $N$ observations is
$$
p(\mu\mid y_1,\ldots,y_N) \propto p(y_1,\ldots,y_N\mid\mu)p(\mu).
$$
Here, as the observations are independent, we have
$$
p(y_1,\ldots,y_N\mid\mu) = \prod_{i=1}^N p(y_i\mid\mu) = \prod_{i=1}^N \frac{1}{\sqrt{2\pi\sigma_a^2}}\exp\left(-\frac{(y_i-\mu)^2}{2\sigma_a^2}\right) = \frac{1}{(2\pi\sigma_a^2)^{N/2}}\exp\left(-\frac{\sum_{i=1}^N(y_i-\mu)^2}{2\sigma_a^2}\right).
$$
We call this likelihood function -- given the prior distribution, what is the probability of observing the data? The posterior distribution is then
$$
\begin{aligned}
p(\mu\mid y_1,\ldots,y_N) 
&\propto \exp\left(-\frac{\sum_{i=1}^N(y_i-\mu)^2}{2\sigma_a^2}-\frac{(\mu-\mu_0)^2}{2\sigma_0^2}\right) \\
&\propto \exp\left(-\frac{1}{2}\frac{(\mu-\phi)^2}{\sigma^2}\right)
\end{aligned}
$$
where 
$$
\phi = \frac{\sigma_0^2}{\sigma_0^2+N\sigma_a^2}\sum_{i=1}^N y_i + \frac{N\sigma_a^2}{\sigma_0^2+N\sigma_a^2}\mu_0, \qquad \sigma^2 = \frac{\sigma_0^2\sigma_a^2}{\sigma_0^2+N\sigma_a^2}.
$$
