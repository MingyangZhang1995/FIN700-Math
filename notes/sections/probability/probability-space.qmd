...
We will not explore the technical details; the intuition is that $\mathcal{F}$ is the collection of events whose occurrence we can determine from the information available. In the above example, $\mathcal{F}_1$ provides more information than $\mathcal{F}_2$. $\mathcal{F}_2$ only allows us to determine whether it is raining or not, while $\mathcal{F}_1$ allows us to determine the exact weather condition. 
